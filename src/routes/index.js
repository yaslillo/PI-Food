    ...
    try {
        ...
        const apiInfo = await apiUrl.data.results.map((info) => {
            return {
                ...
                stepbystep: info.analyzedInstructions.map((path) =>
                    path.steps.map((pathTwo) => pathTwo.step)
                ),
                image: info.image,
                id: info.id,
...
router.get('/recipes', async (req, res) => {
    ...
    const filteredInfo = await info.filter((d) =>
        d.name.toLocaleLowerCase().includes(name.toLocaleLowerCase())
    );

    filteredInfo.length
        ? res.status(200).json(filteredInfo)
        : res.status(404).send('No hay coincidencias con la busqueda');
});
...
router.post('/recipe', async (req, res) => {
    try {
        const { name, score, resume, stepbystep, healthylevel, image, diet } =
            req.body;
